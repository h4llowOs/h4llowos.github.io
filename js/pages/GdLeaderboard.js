import { fetchGdLeaderboard } from '../gd_content.js';
...
export default {
    ...
    template: `
        ...
        <main v-else class="page-leaderboard-container">
            ...
            <div class="leaderboard-switcher">
                <button class="switch-btn" disabled>GD Leaderboard</button>
                <router-link to="/etohleaderboard" class="switch-btn">Etoh Leaderboard</router-link>
            </div>
        </main>
    `,
    ...
    async mounted() {
        try {
            const [leaderboard, err] = await fetchGdLeaderboard();
            ...
            if (routeUsername) {
                const index = this.leaderboard.findIndex(
                    entry => (entry.user || '').toLowerCase() === routeUsername.toLowerCase()
                );
            ...
            if (this.leaderboard[this.selected]) {
                const username = this.leaderboard[this.selected].user;

                if (
                    !this.$route.params.username ||
                    this.$route.params.username.toLowerCase() !== username.toLowerCase()
                ) {
                    this.$router.replace({
                        path: `/gdleaderboard/${encodeURIComponent(username.toLowerCase())}`
                    });
                }
            }
        } catch (e) {
    ...
    methods: {
        ...
        selectPlayer(index) {
            ...
            if (player) {
                this.$router.push({
                    path: `/gdleaderboard/${encodeURIComponent(player.user.toLowerCase())}`
                });
            }
        },
        ...
        getFlag(name) {
            ...
            if (n.includes('squished')) {
                return 'https://flagcdn.com/w160/tr.png';
            }

            if (n.includes('lilbin') || n.includes('h4llow') || n.includes('malikoonium') || n.includes('goober') || n.includes('loafofbread') || n.includes('loafof') || n.includes('loaf') || n.includes('studio') || n.includes('persom') || n.includes('idioticfrogman')) {
